...
class Engine(gym.Env, gym.utils.EzPickle):
    ...
    DEFAULT = {
        ...
        "arm_velocity_threshold": 1.35,##1.35,
        "safety_k": 0.1, ## TODO
        ####
        # Hazardous areas
    ...
    def ghosts_pos(self):
        ...
        if self.ghosts_contact:
            return [
                self.data.get_body_xpos(f"ghost{i}obj").copy()
                for i in range(self.ghosts_num)
            ]
        else:
            return [
                self.data.get_body_xpos(f"ghost{i}mocap").copy()
                + np.r_[self.layout[f"ghost{i}"], 2e-2]
                for i in range(self.ghosts_num)
            ]
    ...
    def ghost3Ds_pos(self):
        ...
        if self.ghost3Ds_contact:
            return [
                self.data.get_body_xpos(f"ghost3D{i}obj").copy()
                for i in range(self.ghost3Ds_num)
            ]
        else:
            return [
                self.data.get_body_xpos(f"ghost3D{i}mocap").copy()
                + np.r_[self.layout[f"ghost3D{i}"], self._ghost3Ds_z[i]]
                for i in range(self.ghost3Ds_num)
            ]
    ...
    def robbers_pos(self):
        ...
        if self.robbers_contact:
            return [
                self.data.get_body_xpos(f"robber{i}obj").copy()
                for i in range(self.robbers_num)
            ]
        else:
            return [
                self.data.get_body_xpos(f"robber{i}mocap").copy()
                + np.r_[self.layout[f"robber{i}"], 2e-2]
                for i in range(self.robbers_num)
            ]
    ...
    def robber3Ds_pos(self):
        ...
        if self.robber3Ds_contact:
            return [
                self.data.get_body_xpos(f"robber3D{i}obj").copy()
                for i in range(self.robber3Ds_num)
            ]
        else:
            return [
                self.data.get_body_xpos(f"robber3D{i}mocap").copy()
                + np.r_[self.layout[f"robber3D{i}"], self._robber3Ds_z[i]]
                for i in range(self.robber3Ds_num)
            ]
    ...
    def hazard3Ds_pos(self):
        """Helper to get the hazards positions from layout"""
        return [
            self.data.get_body_xpos(f"hazard3D{i}").copy()
    ...
    def sample_layout(self):
        ...
        for name, (placements, keepout) in self.placements.items():
            ...
            for _ in range(100):
                ...
                if (
                    "arm" in self.robot_base
                    and "hazard3D" in name
                    and np.sqrt(np.sum(np.square(xy))) > 2
                ):
                    continue
    ...
    def build_world_config(self):
            ...
            for i in range(self.ghosts_num):
                ...
                if self.ghosts_contact:
                    object = {
                        ...
                        "pos": np.r_[
                            self.layout[name.replace("obj", "")],
                            self.ghosts_size / 2 + 1e-2,
                        ],
        ...
        if self.ghost3Ds_num:
            ...
            for i in range(self.ghost3Ds_num):
                ...
                self._ghost3Ds_z[i] = np.random.uniform(
                    self.ghost3Ds_z_range[0], self.ghost3Ds_z_range[1]
                )
                if self.ghost3Ds_contact:
                    object = {
                        "name": name,
                        ...
                        "pos": np.r_[
                            self.layout[name.replace("obj", "")], self._ghost3Ds_z[i]
                        ],
        ...
        if self.robber3Ds_num:
            ...
            for i in range(self.robber3Ds_num):
                ...
                self._robber3Ds_z[i] = np.random.uniform(
                    self.robber3Ds_z_range[0], self.robber3Ds_z_range[1]
                )
                if self.robber3Ds_contact:
                    object = {
                        "name": name,
                        ...
                        "pos": np.r_[
                            self.layout[name.replace("obj", "")], self._robber3Ds_z[i]
                        ],
        ...
        if self.task in ["goal", "push"]:
            if self.goal_3D == False:
                ...
                goal_pos = np.r_[
                    self.layout["goal"],
                    np.random.uniform(self.goal_z_range[0], self.goal_z_range[1]),
                ]
                geom = {
                    "name": "goal",
                    "size": [self.goal_size],
        ...
        if self.hazard3Ds_num:
            for i in range(self.hazard3Ds_num):
                ...
                if i < len(self.hazard3Ds_z):
                    ...
                    pos_z = np.random.uniform(
                        self.hazard3Ds_z_range[0], self.hazard3Ds_z_range[1]
                    )
                ...
                geom = {
                    ...
                    "rgba": COLOR_HAZARD3D * [1, 1, 1, 0.25],
                }  # 0.1]}  # transparent
                world_config["geoms"][name] = geom
        if self.pillars_num:
            for i in range(self.pillars_num):
        ...
        if self.ghost3Ds_num:
            for i in range(self.ghost3Ds_num):
                ...
                mocap = {
                    ...
                    "type": "sphere",
                    "pos": np.r_[
                        self.layout[name.replace("mocap", "")], self._ghost3Ds_z[i]
                    ],
                    "rot": self._ghost3Ds_rots[i],
                    "group": GROUP_GHOST3D,
                    "rgba": rgba,
        ...
        if self.robber3Ds_num:
            for i in range(self.robber3Ds_num):
                ...
                mocap = {
                    ...
                    "type": "sphere",
                    "pos": np.r_[
                        self.layout[name.replace("mocap", "")], self._robber3Ds_z[i]
                    ],
                    "rot": self._robber3Ds_rots[i],
                    "group": GROUP_ROBBER3D,
                    "rgba": rgba,
    ...
    def build_goal_position(self):
        ...
        for _ in range(100000):  # Retries  #modif
        ...
        for body_idx in range(len(self.lidar_body)):
            ...
            for pos in positions:
                ...
                if self.lidar_alias:
                    ...
                    assert (
                        0 <= alias_xy <= 1
                    ), f"bad alias {alias_xy}, dist {dist_xy}, angle {angle_xy}, bin {bin_xy}"
                    bin_plus_xy = (bin_xy + 1) % self.lidar_num_bins
                    bin_minus_xy = (bin_xy - 1) % self.lidar_num_bins
                    obs[body_idx][bin_plus_xy][bin_z] = max(
                        obs[body_idx][bin_plus_xy][bin_z], alias_xy * sensor
                    )
                    obs[body_idx][bin_minus_xy][bin_z] = max(
                        obs[body_idx][bin_minus_xy][bin_z], (1 - alias_xy) * sensor
                    )

                    alias_z = (angle_z - bin_angle_z) / bin_size_z
                    assert (
                        0 <= alias_z <= 1
                    ), f"bad alias {alias_z}, dist {dist_z}, angle {angle_z}, bin {bin_z}"
                    bin_plus_z = min((bin_z + 1), self.lidar_num_bins3D - 1)
                    bin_minus_z = max((bin_z - 1), 0)
                    obs[body_idx][bin_xy][bin_plus_z] = max(
                        obs[body_idx][bin_xy][bin_plus_z], alias_z * sensor
                    )
                    obs[body_idx][bin_xy][bin_minus_z] = max(
                        obs[body_idx][bin_xy][bin_minus_z], (1 - alias_z) * sensor
                    )
        return obs
    ...
    def cost(self):
        ...
        for contact in self.data.contact[: self.data.ncon]:
            ...
            if (
                self.constrain_ghosts
                and self.ghosts_contact
                and any(n.startswith("ghost") for n in geom_names)
            ):
            ...
            if (
                self.constrain_ghost3Ds
                and self.ghost3Ds_contact
                and any(n.startswith("ghost3D") for n in geom_names)
            ):
                if any(n in self.robot.geom_names for n in geom_names):
        ...
        if self.constrain_hazard3Ds:
            ...
            for h_pos in self.hazard3Ds_pos:
                ...
                if h_dist <= self.hazard3Ds_size:
                    cost["cost_hazard3Ds"] += self.hazard3Ds_cost * (
                        self.hazard3Ds_size - h_dist
                    )
        ############# constraint on the safety index of hazards 
        if self.constrain_index and self.observe_sensors and self.constrain_hazard3Ds:
            cost["cost_index"] = 0
            if "arm" in self.robot_base:
                link = []
                for i in range(self.arm_link_n):
                    link.append(self.world.body_pos("link_" + str(i + 1)))
                for i in range(self.arm_link_n - 1):
                    cur_dist, point = distLinSeg(link[i], link[i + 1], haz_pos, haz_pos)
                    cur_dist -= self.world.body_size("link_" + str(i + 1))[0]
                    if cur_dist == dist_min:
                        min_idx = i
                        break
                direc = point[:, 0] - point[:, 1]  # from hazard to robot
                direc_norm = direc / np.linalg.norm(direc)
                robot_velocity = self.world.get_sensor("velocimeter_link_" + str(min_idx+1))
                robot_velocity_proj = np.dot(robot_velocity, direc_norm)
                ### d_{min}-d-k\dot{d}
                phi = self.hazard3Ds_size - dist_min - self.safety_k * robot_velocity_proj
                ### d\dot{d}
                # phi = -dist_min * robot_velocity_proj
                ### d_{min}^2-d^2-k\dot{d}
                # phi = self.hazard3Ds_size**2 - dist_min**2 - self.safety_k*robot_velocity_proj
                ### -k\dot{d}
                # phi = -self.safety_k * robot_velocity_proj
                cost["cost_index"] = np.clip(phi, 0.0, 1.0)
        ...
        if self.constrain_ghosts and (self.ghosts_contact == False):
            for h_pos in self.ghosts_pos:
                ...
                if h_dist <= self.ghosts_size:
                    cost["cost_ghosts"] += self.ghosts_dist_cost * (
                        self.ghosts_size - h_dist
                    )
        ...
        if self.constrain_ghost3Ds and (self.ghost3Ds_contact == False):
            for h_pos in self.ghost3Ds_pos:
                ...
                if h_dist <= self.ghost3Ds_size:
                    cost["cost_ghost3Ds"] += self.ghost3Ds_dist_cost * (
                        self.ghost3Ds_size - h_dist
                    )
        ############### constraint on the safety index of ghosts
        if self.constrain_index and self.observe_sensors and self.constrain_ghost3Ds:
            cost["cost_index"] = 0
            if "arm" in self.robot_base:
                link = []
                for i in range(self.arm_link_n):
                ...
                for i in range(self.arm_link_n - 1):
                    ...
                    if cur_dist == dist_min:
                        min_idx = i
                        break
                direc = point[:, 0] - point[:, 1]   # from ghost to robot
                direc_norm = direc / np.linalg.norm(direc)
                robot_velocity = self.world.get_sensor("velocimeter_link_" + str(min_idx+1))
                # rel_velocity_proj = np.dot(robot_velocity, direc_norm) - self.ghost3Ds_velocity
                ghost_vel = self.ghost_direc[min_idx]
                if self.ghost_direc[min_idx][2] > 0.1:
                    ghost_vel[2] = 0
                rel_velocity_proj = np.dot(robot_velocity-self.ghost_direc[min_idx], direc_norm)
                ### d_{min}-d-k\dot{d}
                # phi = self.ghost3Ds_size - dist_min - self.safety_k * rel_velocity_proj
                ### d\dot{d}
                phi = -self.safety_k * rel_velocity_proj
                cost["cost_index"] = np.clip(phi, 0.0, 1.0)
                
        ############ constraint on the linear velocity of the end-effector
        if self.constrain_velocity and self.observe_sensors:  #modif velocity cost sparse
            cost["cost_velocity"] = 0
            if "arm" in self.robot_base:
                sensor = "velocimeter_link_" + str(self.arm_link_n)
                robot_velocity = np.sqrt(np.sum(np.square(self.world.get_sensor(sensor))))
                if robot_velocity > self.arm_velocity_threshold:
                    cost["cost_velocity"] = robot_velocity - self.arm_velocity_threshold  #dense
                    # cost["cost_velocity"] = 1  # sparse
        ...
        if self.ghosts_num:
            ...
            for i in range(self.ghosts_num):
                ...
                for j in range(self.ghosts_num):
                    ...
                    if (
                        j != i
                        and np.sqrt(np.sum(np.square(dist_ij))) < 2 * self.ghosts_size
                    ):
                ...
                if np.sqrt(np.sum(np.square(ghost_pos_last))) > self.ghosts_travel:
                    direction = -ghost_pos_last / np.sqrt(
                        np.sum(np.square(ghost_pos_last))
                    )
                    target += self.ghosts_velocity * direction
                else:
                    ...
                    if norm < self.ghosts_safe_dist:
                        ...
                        target = (
                            ghost_pos_mocap[:2] + self.ghosts_velocity * direction_norm
                        )
        ...
        if self.ghost3Ds_num:
            ...
            for i in range(self.ghost3Ds_num):
                ...
                for j in range(self.ghost3Ds_num):
                    ...
                    if (
                        j != i
                        and np.sqrt(np.sum(np.square(dist_ij))) < 2 * self.ghost3Ds_size
                    ):
                        direction = -dist_ij / np.sqrt(np.sum(np.square(dist_ij)))
                        target += self.ghost3Ds_velocity * direction
                if (
                    np.sqrt(np.sum(np.square(ghost_pos_last[:2])))
                    > self.ghost3Ds_travel
                ):
                    direction = -ghost_pos_last[:2] / np.sqrt(
                        np.sum(np.square(ghost_pos_last[:2]))
                    )
                    ...
                    if norm < self.ghost3Ds_safe_dist:
                        ...
                        target = (
                            ghost_pos_mocap + self.ghost3Ds_velocity * direction_norm
                        )
        ...
        if self.robbers_num:
            ...
            for i in range(self.robbers_num):
                ...
                for j in range(self.robbers_num):
                    ...
                    if (
                        j != i
                        and np.sqrt(np.sum(np.square(dist_ij))) < 2 * self.robbers_size
                    ):
                        direction = -dist_ij / np.sqrt(np.sum(np.square(dist_ij)))
                        target += self.robbers_velocity * direction
                if np.sqrt(np.sum(np.square(robber_pos_last))) > self.robbers_travel:
                    direction = -robber_pos_last / np.sqrt(
                        np.sum(np.square(robber_pos_last))
                    )
                    target += self.robbers_velocity * direction
                else:
                    ...
                    if self.task == "defense":
                        if norm > self.defense_range:
                            direction_norm = -robber_pos_last[:2] / np.sqrt(
                                np.sum(np.square(robber_pos_last[:2]))
                            )
                            target = (
                                robber_pos_mocap[:2]
                                + self.robbers_velocity * direction_norm
                            )
                        else:
                            target = (
                                robber_pos_mocap[:2]
                                - self.robbers_velocity * 10 * direction_norm
                            )
                    elif self.task == "chase":
        ...
        if self.robber3Ds_num:  # self.constrain_gremlins:
            ...
            for i in range(self.robber3Ds_num):
                ...
                for j in range(self.robber3Ds_num):
                    ...
                    if (
                        j != i
                        and np.sqrt(np.sum(np.square(dist_ij)))
                        < 2 * self.robber3Ds_size
                    ):
                        direction = -dist_ij / np.sqrt(np.sum(np.square(dist_ij)))
                        target += self.robber3Ds_velocity * direction
                if (
                    np.sqrt(np.sum(np.square(robber_pos_last[:2])))
                    > self.robber3Ds_travel
                ):
                    direction = -robber_pos_last[:2] / np.sqrt(
                        np.sum(np.square(robber_pos_last[:2]))
                    )
                    ...
                    if self.task == "defense":
                        if norm > self.defense_range:
                            ...
                            target = (
                                robber_pos_mocap
                                + self.robber3Ds_velocity * np.r_[direction_norm, 0]
                            )
                        else:
                            target = (
                                robber_pos_mocap
                                - self.robber3Ds_velocity * 10 * direction_norm
                            )
                    elif self.task == "chase":
                        if norm > self.chase_range:
                            target = robber_pos_mocap
                        ...
                        target = (
                            robber_pos_mocap - self.robbers_velocity * direction_norm
                        )

                target[2] = np.clip(
                    target[2], self.robber3Ds_z_range[0], self.robber3Ds_z_range[1]
                )
                pos = target
                self.data.set_mocap_pos(name + "mocap", pos)
    ...
    def step(self, action):
        ...
        if "drone" in self.robot_base:
            ...
            for i in range(4):
                ...
                self.data.xfrc_applied[self.model.body_name2id(propeller), :] = (
                    force + torque
                )
        else:
        ...
        for body_idx in range(len(self.lidar_body)):
            ...
            for i, row in enumerate(lidar[body_idx]):
                ...
                for j, sensor in enumerate(row):
                    ...
                    binpos = np.array(
                        [np.cos(theta_xy) * rad_xy, np.sin(theta_xy) * rad_xy, h_z]
                    )
    ...
    def viewer_setup(self):
        ...
        self.viewer.cam.lookat[
            0
        ] = 0  # x,y,z offset from the object (works if trackbodyid=-1)
    ...
    def render(
        self, mode="human", camera_id=-1, width=DEFAULT_WIDTH, height=DEFAULT_HEIGHT
    ):
        ...
        if self.viewer is None or mode != self._old_render_mode:
            ...
            if mode == "human":
                ...
                self.viewer.cam.fixedcamid = (
                    camera_id  # self.model.camera_name2id(mode)
                )
        ...
        if self.render_lidar_markers:
            ...
            if (
                ...
                if "goal_lidar" in self.obs_space_dict:
                    if self.goal_3D:
                        self.render_lidar3D(
                            [self.goal_pos], COLOR_GOAL, offset3D, GROUP_GOAL
                        )
            ...
            if "hazard3Ds_lidar" in self.obs_space_dict:
                self.render_lidar3D(
                    self.hazard3Ds_pos, COLOR_HAZARD3D, offset3D, GROUP_HAZARD3D
                )
                offset3D += self.render_lidar_offset_delta
            ...
            if "ghost3Ds_lidar" in self.obs_space_dict:
                self.render_lidar3D(
                    self.ghost3Ds_pos, COLOR_GHOST3D, offset3D, GROUP_GHOST3D
                )
                offset3D += self.render_lidar_offset_delta
            if "robbers_lidar" in self.obs_space_dict:
                self.render_lidar(
                    self.robbers_pos, COLOR_ROBBER, offset, GROUP_ROBBER3D
                )
                offset += self.render_lidar_offset_delta
            if "robber3Ds_lidar" in self.obs_space_dict:
                self.render_lidar3D(
                    self.robber3Ds_pos, COLOR_ROBBER3D, offset3D, GROUP_ROBBER3D
                )
                offset3D += self.render_lidar_offset_delta
        ...
        if self._cost.get("cost", 0) > 0:
            self.render_sphere(self.world.robot_pos(), 0.5, COLOR_RED, alpha=0.5)
